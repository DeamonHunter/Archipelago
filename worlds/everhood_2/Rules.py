...
def set_hillbert_rules(world: World, valid_types: LocationType, door_keys: bool) -> None: 
    ...
    world.get_location("Floor 23 Reward Chest").access_rule = lambda state: state.has("Floor 23 Key", world.player)
    ...
    world.get_location("Floor Gold Reward Chest").access_rule = lambda state: state.has("Gold Key", world.player)
    if LocationType.cosmetic in valid_types:
        world.get_location("Oingo Boingo").access_rule = lambda state: state.has("Gold Key", world.player)
    
    world.get_entrance("Hillbert Hotel -> Floor Green").access_rule = lambda state: state.has("Green Key", world.player)
    world.get_location("Floor Green Reward Chest").access_rule = lambda state: state.has("Green Key", world.player)
    
    world.get_entrance("Hillbert Hotel -> Floor Pinecone").access_rule = lambda state: state.has("Pinecone Key", world.player)
    world.get_location("Floor Pinecone Reward Chest").access_rule = lambda state: state.has("Pinecone Key", world.player)
    if LocationType.cosmetic in valid_types:
        world.get_location("Reindeer Skull").access_rule = lambda state: state.has("Pinecone Key", world.player)
